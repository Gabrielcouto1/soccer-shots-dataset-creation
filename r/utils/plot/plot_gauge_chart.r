...
plot_gauge_chart <- function(home_prob, draw_prob, away_prob, home_team, away_team){
	...
	outcome_text <- textGrob(
        paste(home_team, " (placeholder) ", away_team, sep=""),
        x = 2, y = 3, just = "centre", gp = gpar(fontsize = 20))

    home_prob_text <- textGrob(
        paste(home_team, " wins ", round(home_prob, 1), "% of times", sep=""),
        x = 0.1, y = 2.5, just = "centre", gp = gpar(fontsize = 13))
    
    away_prob_text <- textGrob(
        paste(away_team, " wins ", round(away_prob, 1), "% of times", sep=""),
        x = 1, y = 2.5, just = "centre", gp = gpar(fontsize = 13))
	...
	text <- paste("These numbers are a result of 10.000 simulations of the match.",
				  ...
				  "and the probability is statsbomb's xG.",
				  "This grafic is inspired by CruzeiroData (x.com/CruzeiroData).", sep="\n")
	
	description <- textGrob(
        text,
        x = -2, y = 1, just = "left", gp = gpar(fontsize = 10))
